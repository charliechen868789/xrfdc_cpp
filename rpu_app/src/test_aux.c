...
void PmTestInit(XScuGic *const GicInst, XIpiPsu *const IpiInst)
{
    ...
    XPm_RequestNode(NODE_IPI_RPU_0, PM_CAP_ACCESS, 100, REQUEST_ACK_BLOCKING);
...
{
	u32 val = pm_read(PMU_GLOBAL_PERS_GLOB_GEN_STORAGE5);
	pm_write(PMU_GLOBAL_PERS_GLOB_GEN_STORAGE5, val |
...
void SyncWaitForReady(const u32 mask)
{
	...
	do
		val = pm_read(PMU_GLOBAL_PERS_GLOB_GEN_STORAGE5) & mask;
	while (val != mask);
}
...
void SyncWaitForValue(const u32 value)
{
	...
	do {
		reg = pm_read(PMU_GLOBAL_PERS_GLOB_GEN_STORAGE5);
	} while (value != reg);
}
...
{
	static u32 binary[] = {
		0xe59f0008,	// ldr r0, PMU_GLOBAL_PERS_GLOB_GEN_STORAGE5
    ...
    //Xil_DCacheInvalidateRange(startAddr, sizeof(binary));
}
#endif
